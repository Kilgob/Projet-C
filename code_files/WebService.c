...
//Lecture de la page web
int ApiWs2(char* AddURL,char *string_curl, int task, char *name_file, char *fileTgrt){
...
//    gtk_combo_box_text_get_active_text(GTK_COMBO_BOX_TEXT(Datas->Target_Serv));
    ...
    printf("uuid file : %s\n",name_file);
    char Str_result[1500];  //À changer selon vos besoin
    char url[140] = "http://";
    char ID_Exec[50];
...
//    strcpy(Str_result, " ");
    while(i != 0){
        ...
        strcat(url, ":");
        strcat(url, json_object_get_string(Datas->Json_conf->PJSON));
        strcat(url, "/tom/cmd/etat_tache?task_id=");
        strcat(url, ID_Exec);
        strcat(url, "&serveur=");
        strcat(url, json_object_get_string(Datas->Json_conf->IP));
        ...
        gtk_label_set_text(GTK_LABEL(Datas->label_Migration_Status), "Dump en cours...");

        if(!strcmp("SUCCESS", json_object_get_string(result))){
            i = 0;
        }
    }
    json_object_object_get_ex(result_buffer, "result", &result);
    printf("\nRetour de l'exécution du programme : %s\n",json_object_get_string(result));
//    result = json_object_new_object();
    
//-------------------------------------------------------------------------------------------------------------------------------------------

    //----------------------------------------------------------------------------------- Requete d'import puis boucle jusqu'au le statut == SUCCES

    //JSON TODO : demande d'import
...
//    strcat(fileDirectory, name_file);//nom du fichier avec l'uuid
    ...
    file_temp = fopen(fileDirectory, "wb");
    migration_serv(Datas, file, "import", name_file);
    migration_datas(Datas->Widgets, file);
    gtk_label_set_text(GTK_LABEL(Datas->label_Migration_Status), "Début de l'import..");
    fprintf(file_temp,"%s",json_object_get_string(file));
//    printf("\ncontenu du fichier : %s\n\n",json_object_get_string(file));
    fclose(file_temp);
    ApiWs(url,Str_result, 1, name_file, fileDirectory);//Ajouter l'uuid pour retrouver le fichier
...
//    strcpy(url, "http://");
    ...
    strcat(url, json_object_get_string(Datas->Json_conf->PJSON));
    strcat(url, "/tom/cmd/exec?cmd=");
    strcat(url, "replicator_c");//peu etre mettre le nom de l'app à exécuter dans le fichier de conf ?
    strcat(url, "%20-f%20"); //passage en parametre du nom du fichier 1/2
    strcat(url, name_file); //passage en parametre du nom du fichier 2/2
    strcat(url, "&serveur=");
    strcat(url, json_object_get_string(Datas->Json_conf->IP));
    ...
    json_object_object_get_ex(result_buffer, "result", &result);
    printf("\nRetour de l'exécution du programme 2 : %s\n",json_object_get_string(result));
//    result = json_object_new_object();
...
//    
    gtk_label_set_text(GTK_LABEL(Datas->label_Migration_Status), "Migration Effectué !");
    remove(fileDirectory);//suppression du fichier tampon stocké sur le disque (crash actuellement)
}




int migration_serv(struct Migration *Serv_info, json_object *file, char *task, char *name_file){
    int i = 0;
    int j = 0;
